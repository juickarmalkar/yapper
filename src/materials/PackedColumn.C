...
registerMooseObject("YapperApp", PackedColumn);
...
PackedColumn::validParams()
{
    ...
    "steel spheres of a specified diameter in accordance with Pamuk and Ozdemir (2012). This also "
    "provides a specified dynamic viscosity of the fluid in the medium.");
    ...
    params.addRangeCheckedParam<Real>(
        ...
        7.98e-04,
        "viscosity != 0",
        "The dynamic viscosity ($\\mu$) of the fluid, the default value is that of water at 30 degree "
        "Celcius (7.98e-04 Pa-s).");
    return params;
}
...
PackedColumn::PackedColumn(const InputParameters & parameters)
    ...
    _viscosity(declareADProperty<Real>("viscosity"))
    {
        ...
        std::vector<Real> permeability = {0.8451e-09, 8.968e-09};
